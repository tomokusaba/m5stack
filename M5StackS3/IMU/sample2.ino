...
// 🎮 IMUデータを更新してカラー計算（ダイナミック版！）
// スムージング用の前回値
static float smoothHue = 0;
static float smoothBright = 0.5f;
static float smoothSat = 0.8f;
...
void updateIMUColor() {
    ...
    if (imu_update) {
        ...
        // 🌈 色相をダイナミックに計算！
        // 方法1: 傾きから直接色相を決定（より反応的）
        float tiltX = atan2(imuAccelX, imuAccelZ);  // -π ~ π
        float tiltY = atan2(imuAccelY, imuAccelZ);  // -π ~ π
        
        // 傾きを色相に変換（傾けると色が大きく変わる！）
        float tiltHue = (tiltX + PI) / (2 * PI);  // X軸傾きをベース
        float tiltOffset = tiltY / PI * 0.5f;      // Y軸傾きでオフセット
        
        // 磁力計も加味（回転で色が変わる）
        float heading = atan2(imuMagY, imuMagX);
        float magHue = (heading + PI) / (2 * PI) * 0.3f;  // 磁力計は30%の影響
        
        float targetHue = fmod(tiltHue + tiltOffset + magHue + 1.0f, 1.0f);
        
        // 🔥 ジャイロで明るさを爆発的に変化！
        float gyroMagnitude = sqrt(imuGyroX * imuGyroX + imuGyroY * imuGyroY + imuGyroZ * imuGyroZ);
        // 感度を10倍に！ちょっと振るだけでMAXになる
        float targetBright = constrain(0.2f + gyroMagnitude / 50.0f, 0.2f, 1.0f);
        ...
        // 振ったり落としたりで彩度が大きく変化
        float targetSat = constrain(0.5f + accelDeviation * 2.0f, 0.5f, 1.0f);
        
        // 🌊 スムージング（急激な変化を少しなめらかに）
        float smoothFactor = 0.3f;  // 0.3 = 結構追従、1.0 = 即座に追従
        smoothHue = smoothHue + (targetHue - smoothHue) * smoothFactor;
        smoothBright = smoothBright + (targetBright - smoothBright) * smoothFactor;
        smoothSat = smoothSat + (targetSat - smoothSat) * smoothFactor;
        
        // 色相は循環するので特別処理
        float hueDiff = targetHue - smoothHue;
        if (hueDiff > 0.5f) hueDiff -= 1.0f;
        if (hueDiff < -0.5f) hueDiff += 1.0f;
        smoothHue = fmod(smoothHue + hueDiff * smoothFactor + 1.0f, 1.0f);
        
        imuBaseHue = smoothHue;
        imuBrightness = smoothBright;
        imuSaturation = smoothSat;
    }
}

// 🎨 IMUベースの色を取得（位置オフセット付き・ダイナミック版）
void getIMUColor(int pixelIndex, float brightnessMod, uint8_t* r, uint8_t* g, uint8_t* b) {
    // 位置に応じた色相オフセット（グラデーション効果を強化！）
    float posOffset = (float)pixelIndex / NUMPIXELS * 0.5f;  // 0.3→0.5に増加
    
    // ジャイロの回転方向で色相を追加シフト
    float gyroShift = (imuGyroZ / 500.0f) * 0.2f;  // Z軸回転で色がシフト
    
    float hue = fmod(imuBaseHue + posOffset + gyroShift + 1.0f, 1.0f);
