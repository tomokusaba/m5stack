...
#define TEMPO_LOCK_THRESHOLD 3      // テンポロックに必要な連続ビート数
#define PHASE_TOLERANCE 0.25f       // 位相許容範囲 (±25% = 整数位置の前後)
...
void drawMicDebug() {
    ...
    if (currentMode != MODE_POV) {
        M5.Lcd.fillRect(220, 5, 100, 20, BLACK);  // BPM表示エリアクリア
        M5.Lcd.setCursor(220, 5);
        M5.Lcd.setTextSize(2);
        M5.Lcd.setTextColor(tempoLocked ? GREEN : DARKGREY);
        if (bpm > 0) {
...
void updateMicFFT() {
    ...
    if (isBeat) {
        ...
        beatDetected *= 0.88f;  // ゆっくり減衰（ビジュアル用）
    }
