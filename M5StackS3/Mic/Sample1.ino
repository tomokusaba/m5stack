/*
 * ============================================================================
 * 🐱 猫耳LED マルチモード スケッチ for M5Stack CoreS3
 ...
 * │   Display: 2.0" IPS LCD (320x240), タッチスクリーン対応                  │
 * └─────────────────────────────────────────────────────────────────────────┘
 * 
 * 【センサー一覧】
 * ┌─────────────────────────────────────────────────────────────────────────┐
 * │ センサー名      │ 型番      │ I2Cアドレス │ 機能                       │
 * ├─────────────────┼───────────┼─────────────┼────────────────────────────┤
 * │ 6軸IMU          │ BMI270    │ 0x69        │ 加速度・ジャイロ           │
 * │ 地磁気センサー  │ BMM150    │ 0x10        │ コンパス（方位）           │
 * │ オーディオコーデック │ ES7210 │ 0x40       │ デュアルマイク入力        │
 * └─────────────────────────────────────────────────────────────────────────┘
 * 
 * 【ピン配置】
 * ┌─────────────────────────────────────────────────────────────────────────┐
 * │ ポート/機能     │ GPIO      │ 接続先                                   │
 * ├─────────────────┼───────────┼──────────────────────────────────────────┤
 * │ PORT.A (赤)     │ GPIO 2    │ Unit Neco (WS2812C-2020 x 70個)          │
 * │                 │ GPIO 1    │ (I2C SDA - 外部デバイス用)               │
 * │ 内部I2C         │ GPIO 12   │ SDA (BMI270, BMM150用)                   │
 * │                 │ GPIO 11   │ SCL (BMI270, BMM150用)                   │
 * │ 内部マイク      │ -         │ ES7210経由 (I2S, 自動設定)               │
 * │ LCDバックライト │ GPIO 45   │ PWM制御                                  │
 * │ タッチパネル    │ -         │ FT6336U (I2C 0x38)                       │
 * └─────────────────────────────────────────────────────────────────────────┘
 * 
 * 【Unit Neco 詳細】
 *   - LED: WS2812C-2020 x 70個（左耳35個 + 右耳35個）
 *   - 電圧: 5V (M5Stack PORT.Aから供給)
 *   - データピン: GPIO 2 (PORT.A)
 *   - 消費電流: 最大約1.4A (全点灯白色時)
 * 
 * 【IMUセンサー → LED制御マッピング】
 *   - 地磁気(方位) → Hue (色相): 北=赤, 東=緑, 南=青, 西=紫
 *   - ジャイロ(回転) → Brightness (明度): 回転速度で明るさ変化
 *   - 加速度(傾き) → Saturation (彩度): 傾きで鮮やかさ変化
 * 
 * 【マイク → LED制御マッピング (FFT分析)】
 *   - 低音域 (60-250Hz)   → 赤系LED、ビート検出 (キック)
 *   - 中音域 (250-2000Hz) → 緑系LED、ビート検出 (スネア)
 *   - 高音域 (2000-4000Hz)→ 青系LED
 *   - FFT: 128サンプル, 8kHz サンプリング
 * 
 * 【ライブラリ依存】
 *   - M5CoreS3.h (M5Stack公式)
 *   - Adafruit_NeoPixel.h (LED制御)
 *   - arduinoFFT.h (音声周波数分析)
 * 
 * ============================================================================
 */
