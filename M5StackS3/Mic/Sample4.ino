...
#define TEMPO_LOCK_THRESHOLD 2      // テンポロックに必要な連続ビート数（2拍でロック）
#define PHASE_TOLERANCE 0.30f       // 位相許容範囲 (±30% = やや広め)
#define EIGHTH_NOTE_ZONE_LO 0.35f   // 8分音符ゾーン下限
#define EIGHTH_NOTE_ZONE_HI 0.65f   // 8分音符ゾーン上限
...
void updateMicFFT() {
    ...
    // --- Step 1: 生のオンセット検出 ---
    if (!isSilent && timeSinceLastBeat > 100) {  // 最小100ms（8分音符@240BPM≒125ms の安全マージン）
    ...
    if (rawOnset) {
        if (!tempoLocked) {
            // --- Step 3a: テンポ未確定 → 自由にビート検出してテンポを学習 ---
            isBeat = true;
            ...
            if (lastBeatTime > 0) {
                ...
                if (rawInterval > 250 && rawInterval < 1000) {
                    ...
                    if (beatCount < 2) {
                        estimatedInterval = rawInterval;  // 最初の数拍は直接設定
                    } else {
                ...
                if (consecutiveOnBeat >= TEMPO_LOCK_THRESHOLD) {
                    ...
                    missedBeats = 0;
                    lastVirtualBeatTime = now;  // 🎯 仮想ビート開始
                    virtualBeatPhase = 0;
                    Serial.println("🔒 TEMPO LOCKED!");
                }
            }
        } else {
            // --- Step 3b: テンポ確定済み → フェーズに基づいて判定 ---
            ...
            if (absPhaseError < PHASE_TOLERANCE) {
                ...
                isQuarterNote = true;
                consecutiveOnBeat++;
                consecutiveOffBeat = 0;  // オンビートなのでオフビートカウンタリセット
                missedBeats = 0;
                ...
                // 🎵 8分音符ゾーン（拍間の中間付近）→ ビジュアルは反応するがテンポ更新しない
                isBeat = true;  // LED反応はさせる
                isQuarterNote = false;  // でも4分音符としては扱わない
                consecutiveOffBeat = 0;  // 8分音符は「予測内」なのでオフビートではない
                Serial.print("♪8th note (phase:");
    ...
    if (isBeat) {
        ...
        // 🎯 テンポロック中: 仮想ビートで予測タイミングにLEDを光らせる
        if (tempoLocked && estimatedInterval > 0 && lastConfirmedBeat > 0) {
            ...
            beatDetected *= 0.5f;  // テンポ未ロック時は普通に減衰
        }
    }
